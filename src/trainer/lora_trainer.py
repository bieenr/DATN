...
from src.util.lr_scheduler import IterExponential, IterConstant
...
class LoraMarigoldTrainer:
    def __init__(
        ...
        lora_layers = filter(lambda p: p.requires_grad, self.model.unet.parameters())
        # Optimizer !should be defined after input layer is adapted
        lr = self.cfg.lr
        self.optimizer = Adam(lora_layers, lr=lr)

        # LR scheduler
        lr_func = IterConstant(
    ...
    def save_checkpoint(self, ckpt_name, save_train_state):
        ...
        unet_lora_state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(self.model.unet))
        StableDiffusionPipeline.save_lora_weights(
            save_directory=lora_path,
            unet_lora_layers=unet_lora_state_dict,
            safe_serialization=True,
        )
        logging.info(f"UNet is saved to: {lora_path}")
    ...
    def load_checkpoint(
        ...
        logging.info(f"Lora parameters are loaded from {_model_path}")

...
class BaseDepthDataset(Dataset):
    ...
    def _augment_data(self, rasters_dict):
        ...
        if random.random() < lr_flip_p:
            bright_aug_para_float = random.random()
            bright_aug_para = int(bright_aug_para_float * 21)
            bright_aug_para_norm = bright_aug_para/ 255.0 * 2.0 
            for k, v in rasters_dict.items():
                if (k == 'rgb_int'):
                    rasters_dict.update({k: torch.clamp(v + bright_aug_para, 0, 255) })
                elif (k =='rgb_norm'):
                    rasters_dict.update({k: torch.clamp(v + bright_aug_para_norm, -1, 1) })
                else:
                    continue
        # if random.random() < lr_flip_p:
        #     crop_height = 1536
        #     crop_width = 1024 
        #     top_float = random.random()
        #     left_float = random.random()
        #     for k, v in rasters_dict.items():
        #         _, h, w = v.shape
        #         top = int((h - crop_height) * top_float)
        #         left = int((w  - crop_width) * left_float)
        #         rasters_dict.update({k: v[:, top:top + crop_height, left:left + crop_width]})
        return rasters_dict
